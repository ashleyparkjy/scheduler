...
(** [get_classes st] is an association list of classes for current
    state [st]. *)
...
(** [take_class st tl] is the user attempting to take a class [tl] as their
    answer to the survey question. The string list of one class
    (ex. ["CS";"3110"]) is concatenated to the classes_input of [st]. *)
...
(** [is_valid_class_time tl] checks if both elements in the string list [tl]
    have valid formats of time. It is true if both satisfy the following
    properties:
...
(** [prompt_lunch st] prompts user to answer question on class spread over the
    week and updates corresponding information to [st]. It also handles any
    commands that are written. *)
...
(** [prompt_class_time st] prompts user to answer preferred class start and end
    times in the day and updates corresponding information to [st]. It also
    handles any commands that are written. *)
val prompt_class_time : t -> t

(** [prompt_class st] prompts user to answer class question and updates 
    corresponding information to [st]. It also handles any commands that are
    written. *)
